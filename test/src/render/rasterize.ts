...
// width, rasterize it to one tall canvas with html2canvas (a DOM walker — it
// doesn't use SVG <foreignObject>, so the canvas isn't tainted and toBlob
// works), then slice that canvas into 256px-tall pages, each cut into a 2×2
// grid of 288×128 tiles that map onto the four image containers.
...
export interface Tile {
    /** Index 0..3, row-major: matches AI_TILE_IDS ordering. */
    index: number;
    x: number;
    y: number;
    width: number;
...
function tileToPng(source: HTMLCanvasElement, sx: number, sy: number): Promise<Uint8Array> {
    ...
    // Solid black background so regions past the document end stay opaque.
    ctx.fillStyle = "#000";
...
/** Render markdown HTML into ordered pages of PNG tiles. */
export async function renderToPages(html: string): Promise<TilePage[]> {
    ...
    root.innerHTML = html;
    // Off-screen but laid out (not display:none, so measurement works).
...
/** Convenience: static geometry for the four image containers on a page. */
