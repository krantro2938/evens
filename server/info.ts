...
//   Gemini    gemini-3.5-flash writes the answer from those chunks and nothing
//             else, and streams it.
//
...
const cfg = {
    ...
    answerModel: process.env.INFO_ANSWER_MODEL ?? "gemini-3.5-flash",
    /**
    ...
    topK: Number(process.env.INFO_TOP_K ?? 6),
    /**
...
function forPanel(text: string): string {
    return (
        text
            ...
            .replace(/[‘’‚‛]/g, "'")
            .replace(/[“”„‟]/g, '"')
            .replace(/[^\p{Script=Cyrillic}\p{Script=Latin}0-9 .,:;!?()[\]«»"'—\-…/%№+=@]/gu, "")
            .replace(/\s{2,}/g, " ")
            .trim()
    );
}
...
const SYSTEM_PROMPT = [
    ...
    "Отвечай ТОЛЬКО по приведённым фрагментам информационных блоков.",
    "Если во фрагментах нет ответа, ответь ровно: В материалах этого нет.",
    "Отвечай по-русски, максимум 40 слов, одним-двумя предложениями.",
    "Без списков, без markdown, без заголовков — только обычный текст.",
 ...
 *
 * maxOutputTokens is 2048 for a forty-word answer, and that is not slack.
...
async function streamAnswer(
    ...
    const res = await fetch(
        `${cfg.geminiBase}/v1beta/models/${cfg.answerModel}:streamGenerateContent?alt=sse`,
        {
            method: "POST",
            headers: { "content-type": "application/json", "x-goog-api-key": cfg.geminiKey },
            body: JSON.stringify({
    ...
    if (!res.ok || !res.body) {
        throw new Error(
            `Gemini ${res.status} (model="${cfg.answerModel}"): ${(await res.text()).slice(0, 400)}`,
        );
    }
...
function splitSource(text: string, chunks: Chunk[]): { body: string; source: string } {
    ...
    // for: a full stop AFTER the bracket ("…31 августа [1].") is the common
    // case and the first version of this regex, which anchored the bracket to
    // the very end, matched none of them — leaving a bare "[1]" on the panel
    // and no citation under it. Several numbers ("[1, 2]") resolve to the
    // first, since one line has room for one source.
    ...
    if (!match) {
        ...
        return { body: text.replace(/\s*\[[\d,\s]*$/, "").trim(), source: "" };
    }
...
export function infoStatus() {
    return {
        ...
        answer: cfg.geminiKey ? cfg.answerModel : null,
...
export function description(): string {
    ...
    return `Инфоблок: ${cfg.sttModel} -> ${cfg.answerModel}, top ${cfg.topK}`;
}
